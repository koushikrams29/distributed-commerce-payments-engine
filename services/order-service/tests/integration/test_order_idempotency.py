...
def test_the_same_items_with_a_new_key_creates_a_separate_order(
    ...
    first = client.post("/orders", json=first_payload)
    second = client.post("/orders", json=second_payload)
...
def test_simultaneous_requests_with_the_same_key_create_one_order(
    ...
    Both threads are held at a barrier so they attempt the insert at the same
    moment. Both see an empty table when they look, so only the constraint can
    stop the second one.
    """
